...
class Obstacle:
    def __init__(self, parent_screen):
        ...
        self.texture = pygame.transform.scale(
            self.texture, (self.height, self.width))
...
# fuctions which operate moving of hero
    def move_up(self):
        ...
        if(self.angle != self.prev_angle):
            self.texture = pygame.transform.rotate(
                self.texture, self.angle-self.prev_angle)
        self.prev_angle = 0
...
class Game:
    ...
    def out_of_range(self, x, y, width, height):
        if(y >= 800-height or y <= 0 or x >= 1000-width or x <= 0):
            return True
        return False
...
# function which operates showing game over screen
    ...
    def show_game_over(self):
        ...
        font = pygame.font.SysFont('arial', 30)
        line1 = font.render(
            f"Game is over: Your level is {self.level}", True, (255, 255, 255))
...
# function which operates gameplay of whole game
    ...
    def play(self):
        ...
        self.end_game.draw()
        self.display_score()
        for i in range(self.hero.num_bombs):
    ...
    # opereting collisions
        ...
        deleted_obst = 0
        for i in range(self.num_obst):
            for j in range(self.hero.num_bombs):
                if(self.is_collision(self.obstacles[i-deleted_obst].x, self.obstacles[i-deleted_obst].y, self.obstacles[i-deleted_obst].width,
                                     self.obstacles[i-deleted_obst].height, self.hero.bombs[j -
                                                                                            deleted_bombs].x, self.hero.bombs[j-deleted_bombs].y, self.hero.bombs[j-deleted_bombs].width,
                                     self.hero.bombs[j-deleted_bombs].height) == True):
                    self.hero.bombs.pop(j)
                    self.obstacles.pop(i)
                    self.num_obst -= 1
                    self.hero.num_bombs -= 1
                    deleted_bombs += 1
                    deleted_obst += 1
        ...
        for j in range(self.hero.num_bombs):
            if(self.out_of_range(
                    self.hero.bombs[j-deleted_bombs].x, self.hero.bombs[j-deleted_bombs].y, self.hero.bombs[j-deleted_bombs].width, self.hero.bombs[j-deleted_bombs].height) == True):
                self.hero.bombs.pop(j)
                self.hero.num_bombs -= 1
                deleted_bombs += 1
